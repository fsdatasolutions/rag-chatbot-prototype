...
function UserChat() {
    ...
    useEffect(() => {
        ...
        const loadMessages = async () => {
            try {
                const res = await axios.get(`/api/chat-sessions/${selectedSessionId}`, {
                    headers: { Authorization: `Bearer ${auth.token}` }
                });
                const formatted = res.data.map(msg => ({ role: msg.role, text: msg.content }));
    ...
    const sendMessage = async () => {
        ...
        console.log('[Sending]', {
            query: input,
            sessionId,
            knowledgeBaseId: selectedKBId || 'none'
        });
    ...
    return (
        <Box sx={{ display: 'flex', height: 'calc(100vh - 64px)' }}>
            ...
            <Box sx={{ flex: 1, display: 'flex' }}>
                ...
                {/* KB Selector */}
                <Box sx={{ width: 280, borderLeft: '1px solid #ddd', p: 2 }}>
